...
async function initListPage() {
  ...
  /* The static per-list shells (pages/list-<slug>.html, generated by
     scripts/generate-list-pages.py) set this attribute so they never need
     a query string — the slug is baked into a real, distinct URL instead.
     Falls back to ?slug= so pages/list.html?slug=x keeps working exactly
     as it always has for anything still linking to it that way. */
  ...
  /* The generated static shell already carries this list's real title
     and description in the raw HTML — this only matters as a fallback
     for pages/list.html?slug=x, which still ships the generic
     placeholder in its <head> until this runs. Belt and suspenders,
     not the primary fix. */
  const metaDesc = document.querySelector('meta[name="description"]');
  if (metaDesc && list.description) metaDesc.setAttribute('content', list.description);

  /* Tells anything that lands on the query-string form where the real,
     canonical page is — the static shell for this exact slug. */
